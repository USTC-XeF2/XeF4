    ...
    if name_or_ip == "-a":
        ...
        for name, status in zip(checked_server.keys(), status_results):
            ...
            if isinstance(status, bool):
                ...
                info += f"{round(status.latency, 1)}ms {status.players.online}/{status.players.max}人在线"
        return info
